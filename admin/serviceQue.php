                ...
                <div class="container-fluid mt-n22 ">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-12">
                            <?php
                            $serviceFilter = isset($_GET['service']) ? $_GET['service'] : ''; // Get service filter value
                            $sqls = $con->prepare("SELECT * FROM appointmenttb WHERE status ='confirmed' " . ($serviceFilter ? "AND service = ?" : "") . " ORDER BY created_at DESC LIMIT 10");
                            if ($serviceFilter) {
                                $sqls->bind_param("s", $serviceFilter);
                            }
                            $sqls->execute();
                                            ...
                                            <div class="row">
                                                <div class="">
                                                    <select name="service" class="form-select" onchange="this.form.submit()">
                                                        ...
                                                        <option value="grooming" <?php if ($serviceFilter == 'grooming') echo 'selected'; ?>>Grooming</option>
                                                        <option value="consultation" <?php if ($serviceFilter == 'consultation') echo 'selected'; ?>>Consultation</option>
                                                        <option value="vaccine" <?php if ($serviceFilter == 'vaccine') echo 'selected'; ?>>Vaccine</option>
                                                        <option value="surgery" <?php if ($serviceFilter == 'surgery') echo 'selected'; ?>>Surgery</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </form>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="table-responsive table-card">
                                    ...
                                    <?php if ($results->num_rows > 0) { ?>
                                        <table class="table text-nowrap mb-0 table-centered table-hover">
                                            <thead style="background-color: #0B1D51;">
                                                <tr>
                                                    <th style="color: whitesmoke;">Queing No.</th>
                                ...
                                <tr>
                                    <td>' . $count++ . '</td>
                                    <td><h5 class=" mb-1">' . $appointmentType . '</h5></td>
                                    <td><h5 class=" mb-1">' . $service . '</h5></td>
                                    <td>' . $date . ' ' . $time_slot . '</td>
                                    <td>' . $ownersName . '</td>
                                    <td>' . $petName . '</td>
                                    <td>' . $contact . '</td>
                                    <td>
                                        <button type="button" class="btn btn-secondary status-btn" data-bs-toggle="modal" data-bs-target="#status" data-appointment-no="' . $appointmentNo . '" data-service="' . $service . '">Status</button>
                                        <button type="button" class="btn btn-success transac-btn" data-bs-toggle="modal" data-bs-target="#static" data-appointment-no="' . $appointmentNo . '" data-service="' . $service . '"><i class="fa-regular fa-thumbs-up"></i> Treated</button>
                                        <a href="updatedTreatedStatus.php?appointmentNo=' . $appointmentNo . '&status=cancel"><button type="button" class="btn btn-danger approv-btn" name="confirmed"><i class="fa-regular fa-circle-xmark" name="rejected"></i> Cancel</button></a>
                                    </td>
                                </tr>';
                                                }
                                                ?>
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered  modal-dialog-scrollable">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form method="post">
                            <label for="appointmentNo">Choose Appointment No. <span style="color: red;">*</span></label>
                            <select name="appointmentNo" id="appointmentNo" class="form-select">
                                <option disabled selected hidden>Select</option>
                                <?php
                                $sql2 = $con->prepare("SELECT DISTINCT appointmentNo FROM appointmenttb WHERE status='confirmed'");
                                $sql2->execute();
                                $results2 = $sql2->get_result();
                                while ($row2 = $results2->fetch_assoc()) {
                                    $appNo = $row2['appointmentNo'];
                                    echo '<option value="' . $appNo . '">appointment No: ' . $appNo . '</option>';
                                }
                                ?>
                            </select>
                            <input type="hidden" id="userid" name="userid">
                            ...
                            <input id="ownersName" type="text" class="form-control" name="fullname" readonly>
                            <label for="service">Service</label>
                            <input id="service" type="text" class="form-control" name="service" readonly>
                            <label for="petName">Pet Name</label>
                            <input id="petName" type="text" class="form-control" name="petname" readonly>
                            <hr>
                            ...
                            <div class="row">
                                <div class="col-6">
                                    ...
                                    <input type="text" id="medicine" class="form-control" required name="medicine">
                                </div>
                                <div class="col-6">
...
<script>
    $(document).ready(function() {
        ...
        $(".status-btn").click(function() {
            // Get the appointment ID and service from the data attributes
            var appointmentNo = $(this).data("appointment-no");
            var service = $(this).data("service");

            // Show the modal
            var modal = $('#status');
            modal.find('#appointmentNo').val(appointmentNo);
            modal.modal('show');

            // Send AJAX request to fetch appointment details
            $.ajax({
                url: "appointmentDetails.php", // Change to your server-side script URL
                method: "POST",
                data: {
                ...
                success: function(response) {
                    ...
                    $("#appointmentNotxt").val(response.appointmentNo);
                    $("#userid").val(response.userid);
                    $("#ownersName").val(response.ownersName);
                    $("#petName").val(response.petName);

                    // Split services and populate the dropdown
                    var services = response.service.split(", ");
        ...
        // JavaScript to handle form submission and update status
        $('#status-form').submit(function(e) {
            e.preventDefault();
            var appointmentNo = $('#appointmentNo').val();
            var selectedService = $('#service').val();

            // Store selected service to prevent it from being shown again
            selectedServices[selectedService] = true;
...
<script>
    document.getElementById('appointmentNo').addEventListener('change', function() {
        var appointmentNo = this.value;
