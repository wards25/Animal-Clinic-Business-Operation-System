...
<body class="bg-light">
    ...
    ?>
    <?php
    $sql = "SELECT * FROM userstb WHERE usertype='standard_User'";
    $query = $con->query($sql);
                ...
                <div class="container-fluid mt-n22">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-12">
                            <!-- Page header -->
                            <div class="mb-5">
                                <h3 class="mb-0" style="color: white;">Chat Room</h3>
                            </div>
                        </div>
                    </div>
                    <div>
                        <div class="card">
                            <div class="row g-0">
                                    ...
                                    <div class="chat-container">
                                        ...
                                        if ($query->num_rows > 0) {
                                            ...
                                            while ($row = $query->fetch_assoc()) {
                                                // Output the user ID and username
                                                echo '<div class="container-inner">';
                                                if (isset($row['imageData'])) {
                                                    echo ' <img style="margin-top:10px; margin-bottonm:10px; margin-left:10px; height:50px; width:50px; border-radius:50%; object-fit:cover;" id="selectedImage" src="data:image/jpg;charset=utf8;base64,' . base64_encode($row['imageData']) . '" />';
                                                } else {
                                                    echo ' <img id="selectedImage" src="../img/pngwing.com.png" style="margin-top:10px; margin-bottonm:10px; margin-left:10px; height:50px; width:50px; border-radius:50%; object-fit:cover;"/>';
                                                }
                                                echo '
                                        <a onclick="loadChat(' . $row['userid'] . ')"><h4  style="display-style:none; color:black; width: 200px; margin-top:20px; margin-left:10px;">' . $row['username'] . '</h4></a></div>';
                                            }
                                        } else {
                                            echo "No users found.";
                                        }
                                ...
                                <div class="col">
                                    <div id="chat-box">
                                        <h3 style="text-align: center; margin-top: 170px;"> <i class="fa-solid fa-message fa-lg" style="color: #1a4389;"></i> Connect With Users</h3>
                                    </div>
                                    <div class="input-group mb-3" style="width: 400px; margin-top:10px; margin-left:10px;">
                                        <input type="hidden" id="receiver_id" name="receiver_id" value="">
                                        <input class="form-control" type="text" id="message" placeholder="Type your message">
                                        <span class="input-group-text" id="basic-addon1"><i onclick="sendMessage()" class="fa-solid fa-paper-plane" style="color: #09409f; cursor:pointer;"></i></span>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    function loadChat(userid) {
        // Set the receiver ID (userid) in a hidden input field
        document.getElementById('receiver_id').value = userid;

        // Make an AJAX request to getMessageAdmin.php with the user ID
        $.ajax({
            type: "GET",
            url: "getMessageAdmin.php",
            data: {
                userid: userid // Pass the user ID as data
            },
            success: function(response) {
                // Update the chat-box with the received chat data
    ...
    function sendMessage() {
        ...
        // Get the receiver ID from the hidden input field
        var receiver_id = document.getElementById('receiver_id').value;

        // Check if the message is not empty
        if (message.trim() !== '') {
            // Make an AJAX request to sendMessageAdmin.php
            $.ajax({
                type: "POST",
                url: "sendMessageAdmin.php",
                data: {
                    message: message,
                    userid: receiver_id
                },
            ...
            // If the message is empty, you can display an error message or take appropriate action
